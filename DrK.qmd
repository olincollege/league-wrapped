...
import scraper as s
import gui as g
import league_wrapped_ui as LoL
...
print(round(vscore[0], 2))
```

```{python}
LoL_wrapped_ui = LoL.LoL_wrapped_interface()
```
...
```{python}
g.make_farm_wrap(farm_data=cs)
```

```{python}
g.make_vision_wrap(vs_data=vscore)
```
